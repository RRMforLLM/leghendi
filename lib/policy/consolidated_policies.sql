...
-- Recreate Device Token table with proper access control
DROP TABLE IF EXISTS "Device Token";
...
CREATE INDEX device_token_user_id_idx ON "Device Token" (user_id);

-- Enable RLS
ALTER TABLE "Device Token" ENABLE ROW LEVEL SECURITY;

-- Device Token policies
CREATE POLICY "device_tokens_read" ON "Device Token"
FOR SELECT USING (true);

CREATE POLICY "device_tokens_insert" ON "Device Token"
FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "device_tokens_update" ON "Device Token"
FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "device_tokens_delete" ON "Device Token"
